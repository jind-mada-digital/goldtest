...
class ProductsSearchProvider extends ResultsProvider
{
    public function search()
    {
        ...
        $matchingVariants = []; // we do not variants as it will be duplicate search
    ...
    protected function searchProducts()
    {
        ...
        return $this->searchProductsFromDefaultLocale(); // as user do not added details for other languags
    ...
    protected function searchProductsFromDefaultLocale()
    {
        /*return Product::where('inventory_management_method', 'single')
                      ...
                      ->get();*/
       return Product::published()  //need single product but they have varients
                  ->where($this->productQuery())
                  ->get();
    }
    ...
    protected function productQuery()
    {
        ...
        // we need to include category as well
        return function ($q) {
            $q->where('published', true)
              ->where(function ($q) {
                  $q->where('name', 'like', "%{$this->query}%")
