            ...
            Mail::queue(
                ...
                function ($message) use ($adminMail) {
                    $message->to($adminMail);
		                  ...
		                  //
                try{
				  $user=$result->order->customer->user;
                   if($user && $user->goldtest_is_school_member &&  in_array($user->goldtest_school_id,[179,163]) ){
                      Mail::queue($this->template('offline.mall::admin.checkout_succeeded_vendor'), $data,
                              function ($message){
                                   $message->to("janine.kaegi@snowsports.ch");
                               });
